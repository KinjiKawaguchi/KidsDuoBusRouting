        ...
        if action == 1:
            ...
            if crud_action == 1:  # Create
                ...
                if pickup_points is not None:
                    id_list = co.receive_multiple_str_input(
                        "更新したいデータのIDを入力してください: ")
                    for id in id_list:
                        current_name = fo.get_pickup_point(id)[1]
                        current_address = fo.get_pickup_point(id)[2]
                        current_is_origin = fo.get_pickup_point(id)[3]
                        current_can_wait = fo.get_pickup_point(id)[4]

                        print(
                            f"ID {id} \n名前: {current_name}\n住所: {current_address} \n教室データ:{current_is_origin}\n待てる場所か:{current_can_wait}")

                        new_name = co.receive_single_str_input(
                            f"ID {id} の新しい名前を入力してください: ")
                        new_address = co.receive_single_str_input(
                            f"ID {id} の新しい住所を入力してください:")
                        new_can_wait = co.receive_single_str_input(
                            f"ID {id} の待てる場所かを入力してください: ")
                ...
                if route_segments is not None:
                    ...
                    for id in id_list:
                        origin_name = fo.get_pickup_point(
                            fo.get_route_segment(id)[5])[1]
                        destination_name = fo.get_pickup_point(
                            fo.get_route_segment(id)[6])[1]
                        current_duration = fo.get_route_segment(id)[3]
                        current_distance = fo.get_route_segment(id)[4]

                        print(
                            f"ID {id} の出発地点は {origin_name}で、到着地点は{destination_name} です。")
                        print(
                            f"現在登録されている所要時間は {current_duration}分で、現在登録されている距離は{current_distance} km です。")

                        new_duration = co.receive_single_str_input(
                            f"ID {id}に対して新しく登録する所要時間を入力: ")
                        new_distance = co.receive_single_str_input(
                            f"ID {id}に対して新しく登録する距離を入力: ")

                        fo.update_route_segment(id, new_duration, new_distance)
            elif crud_action == 3:
                print("終了します。")
                sys.exit()
        elif action == 4:  # Exit
